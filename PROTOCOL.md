...
We proceed to dive into the inner workings of the protocol for a technical audience. The following figure provides an overview of the protocol, which will be explained by the remaining document in more detail.
...
Currently, the Hub will do so by offloading this computation into a special _Registry Enclave_. This is a single enclave which is runs a special _registry image_, and is not intended to be shut down, such that its public key always stays the same. Its main job is to verify the attestation documents of fresh enclaves, which connect to the registry enclave right at launch and submit their attestation document. The registry enclave verifies that the documents are valid and contain the right image hash, extract their public keys, sign it in an EVM-friendly way, and return the signature to the fresh enclave. The fresh enclave can now register its public key with the Hub, which can verify that it is signed with the (hardcoded) public key of the (first) registry enclave. Before trusting the protocol, users can just check this public key once - off-chain - by verifying the attestation document of the first registry enclave. Anyone can launch an additional registry enclave by obtaining a signature of its public key from an existing one (just like an oracle enclave would), and submitting its signed public key to the core contracts of the protocol.
