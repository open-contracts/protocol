...
Yes, in a key way it's not, because we trust that AWS's enclaves are actually enclaves. But we're convinced that it is currently the right trade-off for most smart contracts who want to obtain proofs about internet data, so we need to make it accessible to everyone. 
